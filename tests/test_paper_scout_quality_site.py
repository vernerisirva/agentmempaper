...
class PaperScoutQualitySiteTest(unittest.TestCase):
    def test_quality_assessment_is_rendered_in_html_json_csv_and_detail_page(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertIn("Quality pending", index_html)
            self.assertNotIn("42/100", index_html)
            self.assertNotIn('value="quality-desc"', index_html)
            self.assertNotIn('value="combined-desc"', index_html)
            self.assertNotIn(candidate.title, (docs_dir / "index.html").read_text())
            self.assertIn("Scholarly-quality assessment", about_html)
            self.assertIn("Quality pending", detail_html)
            ...
            self.assertFalse(paper["scholarly_quality"]["downranked"])
