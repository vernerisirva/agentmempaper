# Paper Scout Relevance Evaluation - 2026-06-30

- **Precision-like score:** 1.000
- **Recall-like score:** 1.000
- **False positives:** 0
- **False negatives:** 0
- **LLM classifier used:** False
- **Profile:** deep_research

## False Positives

- None

## False Negatives

- None

## Per-Example Decisions

### deep research agent

- **Expected:** relevant
- **Decision:** relevant
- **Score:** 96
- **Reason:** Studies source-grounded research workflows, citation verification, or evidence-backed research reports.
- **Tags:** deep-research-agents, citation-grounding

### autonomous research agent

- **Expected:** relevant
- **Decision:** relevant
- **Score:** 91
- **Reason:** Studies autonomous or deep research agents.
- **Tags:** deep-research-agents

### AI scientist

- **Expected:** relevant
- **Decision:** relevant
- **Score:** 93
- **Reason:** Studies AI-scientist or scientific-discovery agents.
- **Tags:** ai-scientist, deep-research-agents

### automated literature review

- **Expected:** relevant
- **Decision:** relevant
- **Score:** 93
- **Reason:** Studies automated literature-review systems or literature-review agents.
- **Tags:** literature-review, deep-research-agents

### citation verification agent

- **Expected:** relevant
- **Decision:** relevant
- **Score:** 96
- **Reason:** Studies source-grounded research workflows, citation verification, or evidence-backed research reports.
- **Tags:** citation-grounding, deep-research-agents

### multi-agent research workflow

- **Expected:** relevant
- **Decision:** relevant
- **Score:** 93
- **Reason:** Studies multi-step planning, multi-agent workflows, or hypothesis and experiment-design agents for research.
- **Tags:** multi-agent-research, deep-research-agents

### hypothesis and experiment agent

- **Expected:** relevant
- **Decision:** relevant
- **Score:** 93
- **Reason:** Studies multi-step planning, multi-agent workflows, or hypothesis and experiment-design agents for research.
- **Tags:** hypothesis-experiment, deep-research-agents

### research-agent memory

- **Expected:** relevant
- **Decision:** relevant
- **Score:** 96
- **Reason:** Studies research-agent memory or iterative research state.
- **Tags:** deep-research-agents, research-memory

### generic deep learning

- **Expected:** irrelevant
- **Decision:** irrelevant
- **Score:** 10
- **Reason:** Excluded: not clearly about autonomous research agents or source-grounded research workflows.
- **Tags:** excluded-research-sense

### generic AutoML

- **Expected:** irrelevant
- **Decision:** irrelevant
- **Score:** 10
- **Reason:** Excluded: explicitly lacks autonomous research-agent or citation-grounded workflow focus.
- **Tags:** none

### market research

- **Expected:** irrelevant
- **Decision:** irrelevant
- **Score:** 10
- **Reason:** Excluded: not clearly about autonomous research agents or source-grounded research workflows.
- **Tags:** excluded-research-sense

### autonomous driving

- **Expected:** irrelevant
- **Decision:** irrelevant
- **Score:** 10
- **Reason:** Excluded: explicitly lacks autonomous research-agent or citation-grounded workflow focus.
- **Tags:** none

### generic chatbot

- **Expected:** irrelevant
- **Decision:** irrelevant
- **Score:** 10
- **Reason:** Excluded: not clearly about autonomous research agents or source-grounded research workflows.
- **Tags:** excluded-research-sense

### generic RAG

- **Expected:** irrelevant
- **Decision:** irrelevant
- **Score:** 10
- **Reason:** Excluded: explicitly lacks autonomous research-agent or citation-grounded workflow focus.
- **Tags:** none

### generic benchmark

- **Expected:** irrelevant
- **Decision:** irrelevant
- **Score:** 10
- **Reason:** Excluded: explicitly lacks autonomous research-agent or citation-grounded workflow focus.
- **Tags:** none
