...
""",
    ...
    def test_duplicate_title_variants_are_collapsed_with_sources_and_urls_preserved(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertRegex(index_html, r"Internal screening score: \d+/100")
            ...
            self.assertIn("Review metadata", index_html)
...
""",
    ...
    def test_paper_cards_include_required_fields_and_secondary_metadata(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertIn("More metadata", html)
            self.assertIn("Review metadata", html)
            self.assertIn("Internal screening score: 94/100", html)
            self.assertIn("Screening category", html)
            ...
            self.assertIn('<p class="empty no-results" id="no-results" hidden aria-live="polite">No papers match the current search.</p>', html)
...
""",
    ...
    def test_about_page_and_exports_are_generated(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertIn("relevance screening", about_html.lower())
